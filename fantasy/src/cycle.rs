...
impl Config {
  ...
  pub fn template_path        (&self) -> &PathBuf { &self.template_path        }
...
impl Renderer {
  pub fn render<S: AsRef<str>, P: AsRef<Path>>(&self, tpl_file: S, write_to: P, context: &mut Context) {
    ...
    if !write_to.exists() {
      ...
      if write_dir.is_none() { panic!("Cant not get write dir"); }
