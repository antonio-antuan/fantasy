...
impl<'a> TGClient<'a> {
    ...
    fn walk_dir(&self, dir: &PathBuf, context: &mut Context) {
        ...
        for path in dir.read_dir().unwrap() {
            ...
            let file_path = path
                ...
                .to_str()
                .unwrap();
            let out_file_path = self.cycle.config().output_dir().join(
                path.strip_prefix("./template/rust-tdlib")
                    .unwrap()
                    .as_os_str()
                    .to_str()
                    .unwrap(),
            ...
            self.cycle
                .renderer()
                .render(file_path, out_file_path, context);
        }
    }
    ...
    fn gen_types(&self) {
        ...
        for token in tokens {
            ...
            self.cycle.renderer().render(
                "rust-tdlib/types/td_type.rs",
                self.cycle
                    .config()
                    .output_dir()
                    .join(&format!("types/{}.rs", file_name)[..]),
                &mut context,
            );
        }
    }
    ...
    fn make_context(&self) -> Context {
        ...
        for token in tokens {
            ...
            let file_name = tknwrap.which_file(token.name());
            let mut vec_of_file_obj = file_obj_map
                .get(&file_name)
                .map_or(vec![], |v: &Vec<TLTokenGroup>| v.clone());
